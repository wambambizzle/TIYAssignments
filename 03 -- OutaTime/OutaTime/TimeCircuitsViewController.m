...
@interface TimeCircuitsViewController ()
{
    ...
    NSInteger currentSpeed; // intereger are not objects so no pointers *
}
...
- (void)viewDidLoad
{
    ...
    self.presentTimeLabel.text = [formatDate stringFromDate:[NSDate date]];
...
- (void)destinationDateWasChosen:(NSDate *)destinationDate
{
    ...
    //
    self.destinationTimeLabel.text = [formatDate stringFromDate:destinationDate];
}
...
- (void)startTimer
{
    ...
    //
    if (!speedTimer) // if speed timer object has an no object in it
    {
        //
        ...
        speedTimer = [NSTimer scheduledTimerWithTimeInterval:0.1
                                                      target:self
